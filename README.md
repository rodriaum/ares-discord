...
Este repositório contém o código-fonte de um bot para Discord que permite criar chats privados para geração de imagens e conversas. O bot utiliza as versões mais recentes dos modelos de texto e imagem do OpenAI e Anthropic Deepseek. O histórico das conversas é armazenado em um banco de dados MongoSQL, mas não pode ser utilizado para corrigir as imagens geradas.
...
- **Histórico de conversas**: O histórico das conversas e imagens geradas é armazenado em um banco de dados MongoSQL.
...
- **MongoDB**: O banco de dados utilizado é o MongoSQL (MongoDB), que deve estar configurado e acessível.
...
- **Criar Chat Privado**: Para criar um chat privado, basta enviar o comando `/start` no Discord e o bot criará um canal privado para você interagir com ele.
...
- **MongoSQL**: O banco de dados utilizado é o MongoSQL, que armazena tanto as conversas quanto as imagens geradas.
- **Pacotes NuGet**:
    - `Anthropic.SDK`: Para interação com os modelos de texto da Anthropic.
    - `Ater.DeepSeek.Core`: Para interação com a API DeepSeek para geração de imagens.
    - `Discord.Net`: Para integração com a API do Discord.
    - `DotNetEnv`: Para ler variáveis de ambiente.
    - `Lombok.Nte`: Utilizado para simplificar o código de configuração e injeção de dependência.
...
Se você tiver algum problema ou dúvida, sinta-se à vontade para abrir uma *issue* no repositório.