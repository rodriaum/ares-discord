...
public class SystemMonitor
{
    ...
    private async Task SendStatsAsync()
    {
        ...
        string statsMessage = $"Statistics\nCPU (Últimos 30m, 15m, 5m): {GetAverage(_cpuUsageHistory, 6):F2}%, {GetAverage(_cpuUsageHistory, 3):F2}%, {GetAverage(_cpuUsageHistory, 1):F2}%\n" +
                              $"RAM (Últimos 30m, 15m, 5m): {FormatterUtil.FormatRam(GetAverage(_ramUsageHistory, 6))}, {FormatterUtil.FormatRam(GetAverage(_ramUsageHistory, 3))}, {FormatterUtil.FormatRam(GetAverage(_ramUsageHistory, 1))}";
